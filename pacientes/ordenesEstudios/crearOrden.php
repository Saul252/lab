...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const minSelect   = document.getElementById('minutos');
  const hiddenHora  = document.getElementById('hora_cita');

  const HORA_INICIO = 8;
  const HORA_FIN    = 18;
  const INTERVALO   = 5; // ✅ ahora de 5 en 5

  function hoyISO() {
    return new Date().toISOString().split('T')[0];
  }

  function hayMinutosValidos(hora) {
    const ahora = new Date();
    for (let m = 0; m < 60; m += INTERVALO) {
      if (m > ahora.getMinutes()) return true;
    }
    return false;
  }
  ...
  function cargarHoras() {
    ...
    const ahora = new Date();
    const esHoy = fechaInput.value === hoyISO();
    ...
    for (let h = HORA_INICIO; h <= HORA_FIN; h++) {
      ...
      if (esHoy) {
        ...
        // ⛔ misma hora pero ya no hay minutos disponibles
        if (h === ahora.getHours() && !hayMinutosValidos(h)) continue;
      }

      const hh = String(h).padStart(2, '0');
      horaSelect.innerHTML += `<option value="${hh}">${hh}</option>`;
    }
  }
  ...
  function cargarMinutos() {
    ...
    if (!horaSelect.value || !fechaInput.value) return;

    const ahora = new Date();
    const esHoy = fechaInput.value === hoyISO();
    const horaSel = parseInt(horaSelect.value, 10);

    for (let m = 0; m < 60; m += INTERVALO) {

      if (esHoy && horaSel === ahora.getHours()) {
        if (m <= ahora.getMinutes()) continue;
      }

      const mm = String(m).padStart(2, '0');
      minSelect.innerHTML += `<option value="${mm}">${mm}</option>`;
    }
  }

  function actualizarHoraFinal() {
    if (horaSelect.value && minSelect.value) {
      hiddenHora.value = `${horaSelect.value}:${minSelect.value}`;
    }
  }

  fechaInput.addEventListener('change', cargarHoras);
  horaSelect.addEventListener('change', cargarMinutos);
  minSelect.addEventListener('change', actualizarHoraFinal);
    ...
    // FILTRAR tabla de estudios
      ...
      $(document).on('change', '.select-estudio', function() {
        const tr = $(this).closest('tr');
        if (this.checked) tr.addClass('row-selected');
